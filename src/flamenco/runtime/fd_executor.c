...
status_check_tower( ulong slot, void * _ctx ) {
  ...
  if( FD_UNLIKELY( check_tower_same_slot%16384==0 || check_tower_rooted%16384==0 || check_tower_history%16483==0 ) ) {
    FD_LOG_INFO(( "check_tower_same_slot %lu check_tower_rooted %lu check_tower_history %lu", check_tower_same_slot, check_tower_rooted, check_tower_history ));
  }
  ...
  if( fd_txncache_is_rooted_slot_locked( ctx->status_cache, slot ) ) {
  ...
  /* Agave checks against ancestors, we check against slot_history.
     On the Agave side, both ancestors and slot_history are updated
     during new_from_parent().
     The two checks should be equivalent, as long as our status cache
     stores the same set of transactions as Agave, and unrooted slots
     in the status cache are present in both ancestors and slot_history.
   */
