...
after_credit( fd_pack_ctx_t *     ctx,
  ...
  *charge_busy = 1;
  ...
  if( FD_LIKELY( ctx->bank_idle_bitset ) ) { /* Optimize for schedule */
    ...
    int just_vote_time = ((ctx->last_bundle + ctx->bundle_rush_duration)<now) & (now<(ctx->last_bundle + 4L*ctx->bundle_rush_duration));
    int all_out_time   = (ctx->slot_end_ns - ctx->approx_wallclock_ns)<50000000L;
    ...
    int flags = fd_int_if( just_vote_time, FD_PACK_SCHEDULE_VOTE,
                                           FD_PACK_SCHEDULE_BUNDLE |
                                           fd_int_if( (i==0)&no_votes_bank_0, 0,                    FD_PACK_SCHEDULE_VOTE ) |
                                           fd_int_if( all_out_time,           FD_PACK_SCHEDULE_TXN, 0                     ) );
